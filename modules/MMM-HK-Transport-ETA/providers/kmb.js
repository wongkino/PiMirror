...
class KMBProvider extends HKTransportETAProvider {
	constructor() {
		super();
		...
		if (
			...
			stopResult.value.data.forEach((eta) => {
				if (
					...
					!this.config.routes.includes(eta.route)
				) {
					return;
				}
				if (
					this.config.dirs?.length &&
					!this.config.dirs.includes(eta.dir)
				) {
					return;
				}
				if (eta.eta) {
					const key = `${eta.route}-${eta.dir}-${eta.dest_tc}-${eta.eta}`;
					const existing = uniqueETAs.get(key);
					if (
						!existing ||
						eta.service_type < existing.service_type
					) {
						uniqueETAs.set(key, eta);
					}
				}
			});
			return [...uniqueETAs.values()];
		}
		return [];
	}
	...
	async getKmbStoppings(stopId) {
		...
		try {
			...
			Log.log(
				`KMB ETA Provider: Raw getRoutesInStop response:`,
				JSON.stringify(routesInStopResponse),
			);
	 ...
	 */
	getHeader() {
		const currentETA = this.currentETA();
		const hasActive =
			Array.isArray(currentETA) &&
			currentETA.some(
				(f) =>
...
HKTransportETAProvider.register("kmb", KMBProvider);
