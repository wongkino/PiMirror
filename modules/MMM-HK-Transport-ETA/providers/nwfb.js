...
class NWFBProvider extends HKTransportETAProvider {
	constructor() {
		super();
	...
	generateETAObject(currentETAArray) {
		return currentETAArray.map((currentETA) => {
			...
			const groupedETA = Object.groupBy(
				currentETA,
				(product) => product.dir,
			);
			const value = Object.values(groupedETA)[0];
			return {
				line: value[0].route,
...
HKTransportETAProvider.register("nwfb", NWFBProvider);
