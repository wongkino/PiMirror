...
class LRTProvider extends HKTransportETAProvider {
	constructor() {
		super();
	...
	generateETAObject(currentETAData) {
		...
		return currentETAData.platform_list.map((routeLists) => {
			...
			const groupedRouteList = Object.groupBy(
				routeLists.route_list,
				(product) => product.route_no,
			);
...
HKTransportETAProvider.register("lrt", LRTProvider);
