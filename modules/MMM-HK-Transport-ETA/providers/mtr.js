...
class MTRProvider extends HKTransportETAProvider {
	constructor() {
		super();
	...
	generateETAObject(currentETAData) {
		...
		const etas = Object.keys(etaObject)
			...
			.map((direction) => {
				const groupedByDest = Object.groupBy(
					etaObject[direction],
					(eta) =>
						this.config.mtrData[lineCode].stations.find(
							(station) => station.code === eta.dest,
						)[lang],
				);
				return Object.entries(groupedByDest).map(([key, value]) => ({
					dest: key,
	...
	getParams(stationInfo) {
		return (
			"?" +
			new URLSearchParams({
				line: stationInfo.line_code,
				sta: stationInfo.station_code,
			}).toString()
		);
	}
}

HKTransportETAProvider.register("mtr", MTRProvider);
