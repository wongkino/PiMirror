...
/** HKPlayerFocusHandler */
class HKPlayerFocusHandler {
  constructor(
    configObject,
    devicesEventStream,
    translate,
    ...
    this.playerFocusState = false;
    this.notificationsListenTo = ["PLAYER_FOCUS_STATUS"];

    this.deviceConfig = {
      name:
        configObject.name && typeof configObject.name === "string"
          ? configObject.name.substring(0, 64)
          : "Player Focus",
      serviceName:
        configObject.serviceName && typeof configObject.serviceName === "string"
          ? configObject.serviceName.substring(0, 64)
          : "Player Focus SRV",
    };
    ...
    this.deviceConfig.helperRef = {
      notificationName: "PLAYER_FOCUS",
      icon: "toggle",
      description: "Black out all modules except NowPlaying.",
      docsUrl: "https://github.com/Fabrizz/MMM-HomeKit",
    };

    devicesEventStream.on("__PLAYER_FOCUS_STATUS", (payload) => {
      if (payload && payload.type === "internal") {
        const on = !!payload.on;
        this.playerFocusState = on;
        sendToBackend("PLAYER_FOCUS", on);
      }
    });

    devicesEventStream.on("PLAYER_FOCUS", (payload) => {
      if (payload && payload.type === "set") {
        if (payload.to !== this.playerFocusState) {
          this.playerFocusState = !!payload.to;
          sendNotification("PLAYER_FOCUS_SET", { on: this.playerFocusState });
        }
      }
    });
  }
      ...
      if (payload.type === "set") {
        ...
        if (state !== this.currentState[0]) {
          ...
          if (state) {
            ...
            // Apple Home often turns the light off with brightness 0 and
            // turns it back on without re-sending brightness — that leaves
            // MMM-Remote-Control's black overlay covering the mirror.
            const restoreBrightness =
              brightness > 0 ? brightness : this.currentState[1] || 100;
            this.currentState[1] = restoreBrightness;
        ...
        } else if (brightness !== this.currentState[1]) {
...
class HKPageControlHandler {
  constructor(
    ...
    this.currentPage = 0;
    this.notificationsListenTo = ["NEW_PAGE"];

    const configuredPages = Array.isArray(configObject.pages)
      ? configObject.pages
      : Array.isArray(configObject.pageList)
        ? configObject.pageList
        : null;
    ...
    this.deviceConfig = {
      name:
        configObject.name && typeof configObject.name === "string"
          ? configObject.name.substring(0, 64)
          : translate("ACCESSORY_PAGECONTROL_NAME"),
      serviceName:
        configObject.serviceName && typeof configObject.serviceName === "string"
          ? configObject.serviceName.substring(0, 64)
          : translate("ACCESSORY_PAGECONTROL_SERVICE"),
      pageList:
        configuredPages && configuredPages.length > 0
          ? configuredPages.map(String).slice(0, 8)
          : ["主頁", "Sonarr"],
      startOnFirst: configObject.startOnFirst !== false,
    };
    ...
    // MagicMirror (swipe / indicator) → HomeKit
    devicesEventStream.on("__NEW_PAGE", (page) => {
      if (typeof page !== "number" || page === this.currentPage) return;
      this.currentPage = page;
      sendToBackend("PAGE_CONTROL", page);
    });
  }
  configuration() {
    return this.deviceConfig;
...
const HKAvailableFeatureHandlers = [
  ...
  ["playerFocus", HKPlayerFocusHandler],
];
